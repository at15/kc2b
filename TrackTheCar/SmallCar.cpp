...
CSmallCar::CAR_ERROR CSmallCar::Init( CCvPicCtrl* camera,CGConfigs* config )
{
    ...
    // find the new point
    m_all_map_point = m_proc.RoadPoint(m_config->map_thin_image.GetImage(),m_car_info.core,m_config->que);
    m_config->all_map_points = m_all_map_point;
...
CSmallCar::MOVE_RESULT CSmallCar::MoveCar(CString& log_str,CString& error_str)
{
    ...
    // reach the end
    if(m_all_map_point.size() == (map_point_index+1)){
        m_car_control.Stop();
    ...
    // move the car
    CvPoint target = m_all_map_point.at(map_point_index);

    double distance=CLine::Distance(m_car_info.head,target);
    ...
    else
    {
        ...
        CVector drct_vec(m_car_info.tail,target);
